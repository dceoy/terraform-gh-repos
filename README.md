...
The default repository policy enables Issues, Projects, Wiki, merge/squash/rebase merging, auto-merge, branch updates, deletion of merged branches, and vulnerability alerts. It grants read and write permissions to the default `GITHUB_TOKEN` and does not permit GitHub Actions to approve pull requests by default (this repository opts in explicitly via its own tfvars). Repository rulesets are enabled by default and protect the default branch from deletion and force pushes, require changes through pull requests with zero mandatory approvals, allow merge/squash/rebase, and do not require linear history or review-thread resolution.
